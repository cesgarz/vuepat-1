    ...
    <v-container fluid grid-list-md text-xs-center>
        ...
        <v-card>
            ...
            <template slot="items" slot-scope="item">
                ...
                <td class="text-xs-left">{{ item.item.rol.nb_rol }}  {{ item.item.menu.nb_menu }}</td>
        ...
        <form-container :nb-accion="nb_accion" :modal="modal" @cerrarModal="cerrarModal">
            <sector-form :accion="accion" :item="item" @cerrarModal="cerrarModal"></sector-form>
        </form-container>

        <dialogo 
            :dialogo="dialogo" 
            :mensaje="'Desea Eliminar el Sector: ' + item.rol.nb_menu + ' ' + item.menu.nb_menu "
            @delItem="delItem"
            @delCancel="delCancel"
        >
        </dialogo>
...
export default {
    ...
    return {
        headers: [
        { text: 'Nombre',   value: 'rol.nb_rol menu.nb_menu' },
    ...
    methods:
    {
        list () {
            ...
            axios.get('/api/v1/rolmenu/lista')
            .then(respuesta => {
        ...
        delItem(){
            axios.delete('/api/v1/rolmenu/'+this.item.id_sector)
            .then(respuesta => {
